...
public:
  struct Params {
    bool align_gravity_with_z_axia = false;
    double initialization_window = 2.0;
  ...
  struct ImuState {
    ...
    Eigen::Vector3d mean_specific_force_at_state_origin =
        Eigen::Vector3d::Zero();
  ...
  explicit ContinuousTimeOdometryEstimation(const utils::ParamsNode &config) {
    params_.align_gravity_with_z_axia =
        config.declare<bool>("align_gravity_with_z_axia", false);
    params_.initialization_window = std::max(
        0.0, config.declare<double>("initialization_window", 2.0));
    params_.initialization_min_samples = std::max(
        1, config.declare<int>("initialization_min_samples", 200));
    params_.gravity_norm =
        config.declare<double>("gravity_norm", 9.80665);
    if (!std::isfinite(params_.gravity_norm) || params_.gravity_norm <= 0.0) {
      params_.gravity_norm = 9.80665;
    }
  ...
  [[nodiscard]] const EstimationState &state() const noexcept { return state_; }
  [[nodiscard]] const std::vector<ImuState> &imu_states() const noexcept {
    return imu_states_;
  }
  [[nodiscard]] bool initialized() const noexcept { return inited; }

  void add_point(const common::Point &point,
                 const std::vector<PointCloudSensor> &sensors) {
    (void)point;
    (void)sensors;
    if (!inited) {
    }
  ...
  void add_imu(const common::ImuMsg &imu,
               const std::vector<ImuSensor> &sensors) {
    if (imu.sensor.id >= sensors.size() ||
        !imu.linear_acceleration.allFinite() ||
    ...
    const auto initialize_imu = [](ImuState &state) {
      ...
      state.bg =
          state.gyro_sum / static_cast<double>(state.sample_count);
      ...
      Eigen::Vector3d corrected_specific_force_sum =
          Eigen::Vector3d::Zero();
      ...
      for (std::size_t i = 0; i < state.initialization_samples.size(); ++i) {
        ...
        if (state.initialization_samples.size() >= 2) {
          ...
          if (dt != 0.0) {
            const Eigen::Vector3d omega_first =
                R_state_imu *
                (state.initialization_samples[first].angular_velocity -
                 state.bg);
            const Eigen::Vector3d omega_last =
                R_state_imu *
                (state.initialization_samples[last].angular_velocity -
                 state.bg);
        ...
        const Eigen::Vector3d lever_arm_acceleration =
            alpha_state.cross(r_state_imu) +
            omega_state.cross(omega_state.cross(r_state_imu));
        corrected_specific_force_sum +=
            specific_force_at_imu - lever_arm_acceleration;
      }
    ...
    const auto initialize_state = [this](double timestamp) {
      const auto align_vector_to_z_axis =
          [](const Eigen::Vector3d &vector) -> Eigen::Matrix3d {
        if (!vector.allFinite() || vector.squaredNorm() == 0.0) {
          return Eigen::Matrix3d::Identity();
        }
        return Eigen::Quaterniond::FromTwoVectors(
                   vector.normalized(), Eigen::Vector3d::UnitZ())
            .toRotationMatrix();
      };
      ...
      if (params_.align_gravity_with_z_axia) {
      ...
      utils::log_info(
          ...
          initialized_imus, state_.timestamp, state_.gravity.x(),
          state_.gravity.y(), state_.gravity.z(),
          params_.align_gravity_with_z_axia);
    };
    ...
    if (!imu_state.initialized) {
      ...
      imu_state.initialization_samples.push_back(
          ImuState::InitializationSample{
      ...
      const bool enough_time =
          imu.timestamp - imu_state.first_timestamp >=
          params_.initialization_window;
