...
public:
  inline void setup_subscription(
      ...
      std::function<void(std::vector<common::Point> &, const rclcpp::Time &)>
          callback) override {
    subscription = node->create_subscription<livox_ros_driver2::msg::CustomMsg>(
        ...
        [callback, sensor](const livox_ros_driver2::msg::CustomMsg &msg) {
          ...
          for (const auto &pt : msg.points) {
            if ((pt.tag & 0b00110000) || (pt.tag & 0b00001100) ||
                (pt.tag & 0b00000011)) [[unlikely]] {
              continue;
            }
            cloud.emplace_back();
            auto &p = cloud.back();
            ...
            p.timestamp =
                static_cast<double>(msg.timebase + pt.offset_time) * 1e-9;
            p.sensor = sensor;
          }
