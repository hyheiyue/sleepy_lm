...
public:
  ...
  template <typename T>
  [[nodiscard]] T declare(const std::string &name,
  ...
  template <typename T> [[nodiscard]] T declare(const std::string &name) const {
    const std::string full = prefix_ + name;
...
ParamsNode::declare<Eigen::Vector3d>(const std::string &name) const {
...
ParamsNode::declare<Eigen::Matrix3d>(const std::string &name) const {
  const auto values = declare<std::vector<double>>(name);
  if (values.size() != 9) {
