...
public:
    ...
    [[nodiscard]] double window() const {
        return reorder_window_seconds_;
    }
...
private:
    ...
    struct EntryKeyLess {
        bool operator()(const EntryKey& lhs, const EntryKey& rhs) const {
            return std::tie(lhs.timestamp, lhs.order_key) < std::tie(rhs.timestamp, rhs.order_key);
        }
    };
    ...
    template<typename OnReady>
    void drain_ready(OnReady&& on_ready) {
    ...
    template<typename OnReady>
    void emit_one(OnReady&& on_ready) {
